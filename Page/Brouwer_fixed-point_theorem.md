...
In one dimension, the result is intuitive and easy to prove. The continuous function _f_ is defined on a closed interval [_a_, _b_] and takes values in the same interval. Saying that this function has a fixed point amounts to saying that its graph (dark green in the figure on the right) intersects that of the function defined on the same interval [_a_, _b_] which maps _x_ to _x_ (light green).
