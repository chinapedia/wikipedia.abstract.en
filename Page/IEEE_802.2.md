...
The IEEE 802.2 sublayer adds some control information to the message created by the upper layer and passed to the LLC for transmission to another node on the same data link. The resulting packet is generally referred to as _LLC protocol data unit (PDU)_ and the additional information added by the LLC sublayer is the _LLC HEADER_. The LLC Header consist of _DSAP_ (_Destination Service Access Point_), _SSAP_ (_Source Service Access Point_) and the _Control_ field.
...
IEEE 802.02 Category:Link protocols Category:Logical link control
