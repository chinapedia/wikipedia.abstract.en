...
Logic is the study of the principles of valid reasoning and inference, as well as of consistency, soundness, and completeness. For example, in most systems of logic (but not in intuitionistic logic) Peirce's law (((_P_→_Q_)→_P_)→_P_) is a theorem. For classical logic, it can be easily verified with a truth table. The study of mathematical proof is particularly important in logic, and has applications to automated theorem proving and formal verification of software.

Logical formulas are discrete structures, as are proofs, which form finite trees[15] or, more generally, directed acyclic graph structures[16][17] (with each inference step combining one or more premise branches to give a single conclusion). The truth values of logical formulas usually form a finite set, generally restricted to two values: _true_ and _false_, but logic can also be continuous-valued, e.g., fuzzy logic. Concepts such as infinite proof trees or infinite derivation trees have also been studied,[18] e.g. infinitary logic.
...
In algebraic geometry, the concept of a curve can be extended to discrete geometries by taking the spectra of polynomial rings over finite fields to be models of the affine spaces over that field, and letting subvarieties or spectra of other rings provide the curves that lie in that space. Although the space in which the curves appear has a finite number of points, the curves are not so much sets of points as analogues of curves in continuous settings. For example, every point of the form V(x − c) ⊂ Spec K[x] = 𝔸¹ for K a field can be studied either as Spec K[x]/(x − c) ≅ Spec K, a point, or as the spectrum Spec K[x]_((x − c)) of the local ring at (x-c), a point together with a neighborhood around it. Algebraic varieties also have a well-defined notion of tangent space called the Zariski tangent space, making many features of calculus applicable even in finite settings.
...
-   -   -   -   Ronald Graham, Donald E. Knuth, Oren Patashnik, _Concrete Mathematics_.
...
[1] Richard Johnsonbaugh, _Discrete Mathematics_, Prentice Hall, 2008.

[2]

[3] https://cse.buffalo.edu/~rapaport/191/S09/whatisdiscmath.html accessed 16 Nov 18

[4]

[5] Brian Hopkins, _Resources for Teaching Discrete Mathematics_, Mathematical Association of America, 2008.
...
[19] Graphs on Surfaces, Bojan Mohar and Carsten Thomassen, Johns Hopkins University press, 2001