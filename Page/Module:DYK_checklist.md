...
-- Load modules require('Module:No globals') local yesno = require('Module:Yesno') local data = mw.loadData('Module:DYK checklist/data') local responseIcons = data.responseIcons local statusIcons = data.statusIcons

-- Template for making collapsed sections. local COLLAPSED_WIKITABLE = class="collapsible collapsed" border="1" style="border-collapse:collapse;" |- ! style="font-weight:normal; " | %s |- | %s |}
...
-- Make the given key in args lowercase if when lowercased it equals val. local function makeArgLowerCase(args, key, val)
...
-- Normalize the args table to make it easier to work with elsewhere in the -- module. local function normalizeArguments(args)
...
-- If any of the keys in the keys array are in the table t, return true; -- otherwise, return false. local function anyKeysInTable(t, keys)
...
-- Make a wikitext error message. local function makeWikitextError(msg)

   return string.format(%s, msg)

end

-- Format a user-supplied string for display in error messages. -- This prevents input from being displayed as special wiki markup, converts -- multi-line strings to a single line, and truncates long strings so that they -- are easier to read. local function escapeUserString(s)
...
-- Make a collapsed wikitable with the given header and content. local function makeCollapsedWikitable(header, content)

   return string.format(COLLAPSED_WIKITABLE, header, content)

end

-- Make an uncollapsed wikitable with the given content. local function makeUncollapsedWikitable(content)

   return string.format(UNCOLLAPSED_WIKITABLE, content)

end

-- Make a bulleted list from an array of strings. local function makeBulletedList(items)
...
-- Make a checklist item from the given issue and status. local function makeChecklistItem(issue, status, defaultMarker)
...
-- Return true if all issues have been resolved; return false otherwise. -- mainIssues is an array of tables as passed to makeSection. otherIssues is a -- string value or nil (again, as passed to makeSection). local function allIssuesAreResolved(mainIssues, otherIssues)
...
-- Assemble a section of the DYK checklist. local function makeSection(options)
...
           },
           {
               issue = 'Free of copyright violations, plagiarism, and close paraphrasing',
...
       mainIssues = {
           {
               issue = 'Freely licensed',
               status = args.picfree,
           },
           {
               issue = 'Used in article',
               status = args.picused,
           },
           {
