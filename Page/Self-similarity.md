...
A compact topological space _X_ is self-similar if there exists a finite set _S_ indexing a set of non-surjective homeomorphisms {f_(s) : s ∈ S} for which

_X_ = ⋃_(_s_ ∈ _S_)_f__(_s_)(_X_)

If X ⊂ Y, we call _X_ self-similar if it is the only non-empty subset of _Y_ such that the equation above holds for {f_(s) : s ∈ S}. We call

𝔏 = (_X_, _S_, {_f__(_s_) : _s_ ∈ _S_})
