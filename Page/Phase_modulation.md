...
_c_(_t_) = _A__(_c_)sin (_ω__(_c_)_t_+_ϕ__(_c_)).
,

then the modulated signal is

_y_(_t_) = _A__(_c_)sin (_ω__(_c_)_t_+_m_(_t_)+_ϕ__(_c_)).
...
$$m(t) = \cos\left(\omega_\mathrm{c} t + h\omega_\mathrm{m}(t)\right)\$$
...
,
    ...
    where f_(M) = ω_(m)/2π and h is the modulation index defined below. This is also known as Carson's Rule for PM.
