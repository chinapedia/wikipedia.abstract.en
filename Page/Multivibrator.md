...
If V_(C) is the voltage across the capacitor and from the graph, the time period of the wave formed at capacitor and the output would match, then the time period could be calculated in this way:$V_c = V_c(\infty)+[V_c(0)-V_c(\infty)]e^\tfrac{-t}{RC}$
